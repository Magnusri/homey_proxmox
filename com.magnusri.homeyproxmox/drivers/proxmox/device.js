    ...
    try {
      ...
      if (data.type === 'node') {
        ...
        if (isOnline) {
          ...
          // Nodes use 'memory' (total) and 'memused' fields, not 'mem'/'maxmem'
          if (status.memory !== undefined && status.memory > 0) {
            // Calculate percentage: (memused / memory) * 100
            const memUsed = status.memused || 0;
            memPercent = this.roundToOneDecimal((memUsed / status.memory) * 100);
    ...
    try {
      ...
      if (data.type === 'lxc' || data.type === 'vm') {
        ...
        if (currentNode && currentNode !== data.node) {
          this.log(`${data.type} ${data.vmid} migrated from ${data.node} to ${currentNode}`);
          data.node = currentNode;
          await this.setStoreValue('node', currentNode);
        }
      }
